...
{tr[~tr['below_target']]['idle_days'].mean():.1f} يوم لبقية الأسطول.</p></div>""")
    ...
    dt_ = A.driver_trips(d)
    DTF = {"trips": lambda v: f"{int(v)}", "days": lambda v: f"{int(v)}",
           "trucks": lambda v: f"{int(v)}", "total": lambda v: f"{v:,.0f}",
           "trips_per_day": lambda v: f"{v:.1f}"}
    drivers_tbl = table("النقلات لكل سائق", dt_,
...
<div class="note">النقلات والكميات وأيام العمل لكل سائق. احتساب الحوافز خارج نطاق
هذا التقرير حالياً.</div></div>
...
{drv_flag}
{drv_tbl}
<div class="alert mid"><div class="t">تحفّظ على دقة نسبة الحركات للسائقين</div>
<div class="d">بمقارنة الشيت مع تقرير الحركة الخام لفترة 21/6 – 20/7 تبيّن أن
نحو 5% من الصفوف تحمل اسم سائق مختلفاً، وأن الاختلاف يتركّز في السندات الكبيرة
التي تحوي صف مضخة يكسر تسلسل الأسماء فتنزلق صفاً واحداً. عمودا السائق والخلاطة
ينزلقان معاً فيبقى الشيت متسقاً داخلياً، ولذلك لا يمكن رصد الخلل آلياً من الشيت
وحده. الأثر على الترتيب أدناه محدود لكنه قد يقلب مواقع السائقين الذين تفصل بينهم
فروق صغيرة (أقل من 0.10 لتر لكل متر). اعتمد هذا الجدول للاتجاه العام، وراجع
الحالات الحرجة مع تقرير الحركة الخام قبل أي إجراء فردي.</div></div>
